<template>
  <div class="container">
    <div v-if="docs">
      <v-data-table :headers="headers" :items="docs" sort-by="id" class="elevation-1">
        <template v-slot:top>
          <v-toolbar flat>
                    ...
                    <v-form @submit.prevent="onSend">
                      <v-row>
                        <v-col cols="12" md="6">
                          ...
                          ></v-text-field>
                          <div v-if="!$v.doc.name.required" class="invalid-feedback">The name field is required.</div>
                        </v-col>
                      </v-row>
                    </v-form>
                  </v-container>
...
export default {
  ...
  methods: {
    editItem(item) {
      ...
      this.arrIndex = this.docs.indexOf(item)
      this.doc.name = this.docs[this.arrIndex].name
      this.dialog = true
    },
    ...
    deleteItem(item) {
      this.editedIndex = item.id
      this.arrIndex = this.docs.indexOf(item)
      this.doc = this.docs[this.arrIndex]
    ...
    async deleteItemConfirm() {
      ...
      await axios
        .delete(`/documents-groups/${this.editedIndex}`)
        .then(res => {
          this.docs.splice(this.arrIndex, 1)
        ...
        await axios
          ...
          .then(res => {
            if (res.data.errors) {
              ...
              this.docs.push(res.data.response)
              this.close()
              this.isLoading = false
            }
          })
    ...
    async onUpdate() {
      ...
      if (!this.$v.$invalid) {
        this.isLoading = true
        await axios
          .patch(`/documents-groups/update/${this.editedIndex}`, {
            name: this.doc.name,
          })
          .then(() => {
            this.docs[this.arrIndex].name = this.doc.name
            this.isLoading = false
            this.close()
          })
          .catch(error => {
