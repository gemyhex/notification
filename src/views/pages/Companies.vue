<template>
  <div class="container">
    <div v-if="companies">
      <v-data-table :headers="headers" :items="Companies" sort-by="id" class="elevation-1">
        <template v-slot:top>
          <v-toolbar flat>
...
export default {
  ...
  methods: {
    editItem(item) {
      ...
      this.arrIndex = this.companies.indexOf(item)
      this.company.name = this.companies[this.arrIndex].name
      this.company.client_id = this.companies[this.arrIndex].client.id
      this.company.company_type_id = this.companies[this.arrIndex].company_type.id
      this.dialog = true
    },
    ...
    deleteItem(item) {
      this.editedIndex = item.id
      this.arrIndex = this.companies.indexOf(item)
      this.comapny = this.companies[this.arrIndex]
    ...
    async deleteItemConfirm() {
      ...
      await axios
        ...
        .then(res => {
          this.companies.splice(this.arrIndex, 1)
        ...
        await axios
          ...
          .then(res => {
            if (res.data.errors) {
              ...
              this.companies.push(res.data.response)
              this.close()
              this.isLoading = false
            }
          })
    ...
    async onUpdate() {
      ...
      if (!this.$v.$invalid) {
        ...
        await axios
          ...
          .then(() => {
            this.companies[this.arrIndex].name = this.company.name
            this.isLoading = false
            this.close()
          })
          .catch(error => {
